...
export default function BrandingSettings() {
  ...
  async function load() {
    const { data } = await supabase
      .from("site_settings")
      .select(
        "logo_url, store_image_day_url, store_image_night_url, hero_image_url, store_gallery_urls, social_facebook, social_instagram, social_whatsapp, invoice_barcode_platforms, promo_video_url, promo_video_enabled"
      )
      .eq("id", 1)
      .single();
  ...
  return (
    <main className="home-page space-y-6">
      ...
      <div className="glass-card space-y-4">
        <h2>📊 بارکد شبکه‌های اجتماعی روی فاکتور</h2>
        <p style={{ color: "#6B7280" }}>
          کدام شبکه‌ها به‌صورت بارکد (قابل اسکن) روی فاکتور چاپی/دانلودی چاپ شوند؟
        </p>
      ...
      <div className="glass-card space-y-4">
        ...
        <p style={{ color: "#6B7280" }}>
          لینک مستقیم فایل ویدیو (mp4) یا لینک امبد یوتیوب/آپارات. اگر فعال باشد، در صفحه اصلی نمایش داده می‌شود.
        </p>
