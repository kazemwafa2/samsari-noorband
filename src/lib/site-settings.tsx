...
export function SiteSettingsProvider({ children }: { children: React.ReactNode }) {
  ...
  async function load() {
    ...
    const { data } = await supabase
      .from("site_settings")
      .select(
        "logo_url, store_image_day_url, store_image_night_url, store_gallery_urls, hero_image_url, social_facebook, social_instagram, social_whatsapp, invoice_barcode_platforms, promo_video_url, promo_video_enabled, theme"
      )
      .eq("id", 1)
      .single();
